...
在学习本章前，建议先了解下上一章的 java内存管理

垃圾回收是为了高效使用内存，通过释放可回收对象来释放占用内存空间。

java提供垃圾回收机制，可以让我们高效开发，无需关心程序中内存管理。
...
`m= null;` 当m赋值null后，jack对象引用计数减1，变为0。

这样jack对象变为可回收对象或称为可回收的垃圾

引用计数算法特点是将垃圾回收分摊到整个应用程序的运行当中了，比较高效的

看似很美好的方法，那是什么原因导致我们最终放弃了引用计数算法呢？
...
```
可达性分析算法中，将对象直接或间接挂在引用链上，以GC Roots的对象为起点
当一个对象到GC Roots的对象没有可达的路径时，则任务该对象是可回收的
```
![gc2](gc2.png)

```
通过可达性算法，成功解决了引用计数所无法解决的问题-“循环依赖”，
只要你无法与 GC Root 建立直接或间接的连接，系统就会判定你为可回收对象。
```

那这样就引申出了另一个问题，GC Root的对象有哪些。
...
方法print()局部变量duck，存放在虚拟机栈的栈帧中，应用这个对象的称为GC Root的对象
...
特点：回收高效，但是容易形成内存碎片，导致内存利用低，长期使用内存利用率越来越低

```
上图中等方块的假设是 2M，小一些的是 1M，大一些的是 4M。等我们回收完，内存就会切成了很多段。
我们知道开辟内存空间时，需要的是连续的内存区域，这时候我们需要一个 2M的内存区域，
其中有2个 1M 是没法用的。这样就导致，其实我们本身还有这么多的内存的，但却用不了。
```
...
缺点，虽然只能使用一半的内存空间,有一定的内存浪费，但是不会造成内存利用率越来越低，回收效率比标记清除法慢

### 标记整理算法 - Mark-Compact

标记整理算法，标记过程仍然与标记清除算法一样，
```
先对可回收对象进行标记，然后让所有存活的对象都向一端移动，
再清理可回收对象，这样提供一整块连续内存区域。
```
![gc5](gc5.png)
```
...
特点： 回收效率是三个算法中最慢的，但是内存没有浪费，也不会造成内存利用率越来越低

### 分代收集算法 - Generational Collection

分代收集算法融合上述3种基础的算法思想，而产生的针对不同情况所采用不同算法的一套组合拳
```
对象存活周期的不同将内存划分为几块。一般是把 Java 堆分为年轻代和老年代，
这样就可以根据各个年代的特点采用最适当的收集算法。

年轻代
年轻代是为创建对象而分配空间的，临时性对象占绝大部分，而年轻代内存是限的，需要反复利用
...
```
Java 堆主要分为2个区域-年轻代与老年代，
其中年轻代分 Eden 区和 Survivor 区，其中 Survivor 区又分 From 和 To 2个区。
为什么是这样来分的呢？
```
...
#### Eden 区
```
IBM公司的专业研究表明，有将近98%的对象都是临时使用，针对这一现状，大多数对象会在年轻代 Eden 区中进行分配，
...
```
不就是年轻代到老年代么，直接 Eden 到 Old 不好了吗，为啥要这么复杂。
...
Survivor 的预筛选保证，只有经历16次 Minor GC 还能在年轻代中存活的对象，才会被送到老年代。
```
...
在年轻代这种经常会消亡的区域，采用标记清除必然会让内存产生严重的碎片化。
...
1. Eden区空间不足以分配新的对象时，触发年轻代的垃圾回收，则称为minor gc 次要垃圾回收
2. 老年代空间不足时触发老年代的垃圾回收，则触发major gc 主要垃圾回收
3. 如果同时触发年轻代和老年代的垃圾回收，则称为 full gc
...
1. 触发老年代垃圾回收，不如年轻代对象移到老年代时老年代空间不足、有连续的大对象需要分配
